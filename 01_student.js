...
// ===== NEW: bật Editor V2 (ảnh 2) thay cho editor cũ (ảnh 1)
// Editor V2 chạy trong iframe (editor_v2.html) và đồng bộ bằng postMessage.
document.body.classList.add('useEditorV2');

function _v2Frame(){
  try{ return document.getElementById('editorV2Frame'); }catch(e){ return null; }
}
function sendLessonToEditorV2(){
  const fr = _v2Frame();
...
// Nhận sự kiện từ Editor V2 (PASS/READY)
window.addEventListener('message', (ev)=>{
  const d = ev && ev.data ? ev.data : null;
  if(!d || typeof d !== 'object') return;
  if(d.type === 'EDITOR_V2_READY'){
    // iframe đã sẵn sàng nhận dữ liệu
    sendLessonToEditorV2();
  }
  if(d.type === 'EDITOR_V2_PASS' && d.lessonId){
    // PASS theo cấu trúc từ editor v2 -> dùng lại cơ chế mở khóa cũ
    try{ if(!progress.passed[d.lessonId]) markPassed(d.lessonId); }catch(e){}
  }
}, false);
...
function renderLessonList(){
  ...
  try{ window.current = current; }catch(e){}
      ...
      renderTask();
        renderVideoPanel();
loadProgressFor(l);
      // Trên Editor V2 + màn hình nhỏ: giữ danh sách mở để thao tác nhanh hơn
      try{
        const _isV2 = document.body.classList.contains('useEditorV2');
        const _isSmall = window.innerWidth <= 980;
        if(!(_isV2 && _isSmall)) toggleLessonDrop(false);
      }catch(e){ toggleLessonDrop(false); }
...
/* =========================================================
   ...
   ========================================================= */
function bindEvents(){
  // ===== UX tốt hơn khi dùng Editor V2 (ảnh 2)
  // - Trên màn hình nhỏ: mở sẵn danh sách bài, không tự đóng khi click ra ngoài
  // - Tránh cảm giác "2 lớp cuộn" và phải bấm nhiều lần
  const _isV2 = document.body.classList.contains('useEditorV2');
  const _isSmall = ()=> window.innerWidth <= 980;

  el("lessonHeader").onclick = ()=>{
    if(_isV2 && _isSmall()) return; // giữ mở sẵn trên mobile
    toggleLessonDrop();
  };

  document.addEventListener("click", (e)=>{
    if(_isV2 && _isSmall()) return;
  ...
  // Mở sẵn dropdown nếu đang dùng Editor V2 (đặc biệt trên mobile)
  try{
    if(_isV2 && _isSmall()) toggleLessonDrop(true);
  }catch(e){}
